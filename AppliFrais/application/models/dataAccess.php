...
// TODO : Transformer toutes les requêtes en requêtes paramétrées
	...
	*/
	public function getInfosVisiteur($login, $mdp){
		...
		$req = "select visiteur.idProfil as type
				from visiteur 
				where visiteur.login=? and visiteur.mdp=?";
		$rs = $this->db->query($req, array ($login, $mdp));
		$ligne = $rs->first_row('array'); 
	...
	*/
	...
	/**
	 * Obtient toutes les fiches (sans détail) d'un comptable
	 * 
	 * @param $idVisiteur 
	*/
	public function getFichesComptable () {
		$req = "select idVisiteur, nom, mois, montantValide, dateModif, idEtat, libelle
				from  fichefrais inner join Visiteur on ficheFrais.idVisiteur = Visiteur.id inner Join Etat on ficheFrais.idEtat = Etat.id 
				where idEtat = 'CL'
				order by mois desc";
		$rs = $this->db->query($req);
		$lesFiches = $rs->result_array();
